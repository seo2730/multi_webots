...
### ⑬ 탐사 커버리지가 20% 언저리에서 평평해진다 — 경유점이 제자리로 뭉개진 것

`frontier_allocator`(explorer 컨테이너)는 `/map_merged`에서 프론티어를 뽑아 로봇에
배정한다 (전체 설명은 [12장](12_TASK_ALLOCATION.md)). 그런데 배정한 목표가 **그 로봇 자기 맵 밖**이면 Nav2가 거절하므로,
`clamp()`가 목표를 그 로봇의 계획 가능 범위 안까지 잘라 **경유점**으로 보낸다.

여기에 버그가 있었다. 예전에는 로봇→목표 광선 전체를 스칼라 하나로 줄였다
(`s` = 두 축의 교차 매개변수 중 `min`). 그러면 **한 축의 미미한 여유가 다른 축의
긴 전진을 통째로 죽인다.** 실측:

```
ugv1 (-48.11, -23.42)  →  목표 (-49.6, 41.0)      dx = -1.49,  dy = +64.4
ugv1 맵 좌측 끝 -50.11  →  여백 2 m 를 빼면 x0 = -48.11 = 로봇의 x 그대로
  x축 s 후보 = 0   ←  min 이 이걸 채택
  y축 s 후보 = 1.22
경유점 = (-48.11, -23.42) = 로봇 자기 위치,  전진 0.00 m
```

증상은 조용하다. 에러가 없고, Nav2 로그는 오히려 **성공**처럼 보인다:

```
[ugv1.bt_navigator]: Begin navigating from current location (-48.11, -23.42) to (-48.11, -23.42)
[ugv1.controller_server]: Reached the goal!
[ugv1.bt_navigator]: Goal succeeded
```

로봇이 안 움직이니 맵이 안 자라고, 맵이 그대로니 다음 주기도 같은 결론이 나온다 —
**영구 교착**이다. 할당기 쪽에는 `3주기 동안 진전이 없어 목표를 버린다`만 반복 출력된다.

**고친 방법: 축별로 자른다.** 광선을 스칼라로 줄이는 대신 각 축을 독립적으로 범위
안에 넣는다. 위 사례는 `(-48.11, 41.0)`이 되어 북쪽 64 m 가 살아난다. 더해서 결과가
`min_goal_dist`보다 가까우면(로봇이 맵 구석에 박혀 모든 축이 막힌 경우) 목표 방향으로
최소 전진거리만큼은 밀어 준다 — 코스트맵을 벗어나 Nav2가 거절할 수는 있지만
확실한 교착보다는 낫다.

| | 수정 전 | 수정 후 |
|---|---|---|
| 위 사례의 전진거리 | 0.00 m | 64.4 m |
| 2대 20분 탐사 커버리지 | 19.7% / 13.3% (평평) | **33.3%** (단조 증가) |
| 목표 포기 횟수 | 수십 회 | 11 회 |

> 함께 봐야 할 것: 반복 실험에서 `compose down` 은 월드를 리셋하지 않는다
> ([03장 8절](03_SPAWNER.md#-반복-실험에서-compose-down-은-리셋이-아니다)).
> 위 측정도 처음에는 그 탓에 시행끼리 시작 상태가 달랐다.
