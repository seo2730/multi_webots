...
전에 해당 문서를 먼저 본다.** 문서는 번호 붙은 11개 장으로 정리돼 있고, 책 목차는
[Readme.md](Readme.md#-목차)에 있다 (`01_INTERFACES.md` ~ `11_DATA_COLLECTION.md`).
...
- **호스트에 파이썬이 없다** (Store 스텁만). 파이썬 스크립트는 프로젝트 도커 이미지 안에서
  돌린다 — 셸별 문법은 [02장 2절](02_WORLD_GEN.md#2-os별-실행-방법-중요) 참고.
