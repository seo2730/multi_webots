...
목표 — 이 둘을 **최소화**한다
1. **새로 내주는 목표의 수.** 한 번 보낸 곳에서 많이 탐사되면 다음 목표가 덜 필요하다.
2. **로봇이 이동하는 거리.** 멀리 보내면 그만큼 시간과 거리를 쓴다.

읽는 법
- robots[].sensor / strength: 그 기체가 무엇을 잘 보는가.
- robots[].nav_bounds: 그 로봇이 실제로 경로를 만들 수 있는 범위 [x0,y0,x1,y1].
  이 밖의 후보를 주면 그 로봇은 한 주기를 통째로 낭비한다. 없으면 제한이 없다는 뜻.
- frontiers[].cells: 그 후보에 속한 경계 셀 수. 클수록 한 번에 많이 탐사된다.
- frontiers[].openness: 후보 주변이 얼마나 트였는가(0~1). 높으면 트인 공간이다.
- frontiers[].dist: 로봇별 직선 거리(m).

규칙
1. 한 로봇에 후보 하나. 두 로봇이 같은 후보로 가지 않는다.
2. **가깝고 큰 후보를 고른다.** dist 가 작고 cells 가 큰 것이 좋다 — 적은 이동으로 많이
   탐사되면 목표 수와 이동거리가 함께 준다. 둘이 충돌하면 그 로봇이 무엇을 잘 보는지로 가른다
   (드론은 트인 곳을 빠르게, UGV 는 좁고 복잡한 곳을).
3. 두 로봇이 같은 구역을 중복해서 훑지 않게 배정된 후보를 **{min_sep} m 이상** 떨어뜨린다.
   이것은 부차 목표다 — 거리를 크게 늘려야만 맞출 수 있다면 더 가까운 배정을 택한다.
4. 좌표를 새로 만들지 마라. 반드시 frontiers 목록의 id 중에서 고른다.
5. nav_bounds 밖의 후보는 그 로봇에게 배정하지 마라.
...
# v2 (2026-09-17): 목표를 '분리 최대화' 에서 **목표 수·이동거리 최소화** 로 바꿨다.
#   v1 로 모은 데이터와 섞으면 학생이 보는 지시가 실행마다 달라진다 — 판으로 가른다.
PROMPT_VERSION = 'alloc-v2'
