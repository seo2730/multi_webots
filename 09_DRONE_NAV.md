# 09. 드론 자율비행 — 구조와 직접 테스트하는 법

> 📖 [책 목차](Readme.md#-목차) · ← [08. 드론 구축](08_DRONE_SETUP.md) · [10. 맵 병합](10_MAP_MERGE.md) →

드론이 목표점까지 스스로 날아가고, 앞이 막히면 **고도를 올려 넘어가는** 구조의 설명서.
"무엇을 만들었나 / 왜 그렇게 만들었나 / 어떻게 직접 돌려보나"를 한 곳에 모았다.

기체 자체(개조·제어·센서)는 [08_DRONE_SETUP.md](08_DRONE_SETUP.md), 빠른 사용법은
[Readme 11절](Readme.md#11-drone-중형급-쿼드콥터)에 있다.
...
> 너무 높으면(꼭대기가 안 보이면) 층 단위로 물러난다. 둘 다 [7절 ③④](#7-겪은-함정-모음) 참고.
...
| 값 | 의미 |
|---|---|
| `2d` / `2.5d_local` / `2.5d` | [0절](#0-경로계획-모드-고르기) 참고 |
...
**⓪ 층 격자를 월드 크기에 맞춰 두면 넓은 월드에서 조용히 죽는다.** 예전에는 격자가
상수였다 — `origin (-10, -8)`, `200x160 @ 0.1 m` = **20x16 m**. 작은 아레나(`my_world`)
기준 값이라 `oneroom`(96x96 m)에서 드론이 `(-46.25, 23.75)` 에 소환되자 **기체가 아예
격자 밖**이었다. 센서는 멀쩡한데(하향 뎁스 30 Hz, 라이다 3.8 Hz) **찍을 칸이 없어서**
탐색률이 0% 로 남았고, 층 선택기가 세 층 모두 "미탐색 100%" 로 보고 헛돌았으며, Nav2 는
`Robot is out of bounds of the costmap` 으로 목표를 실패시켰다.

지금은 **창이 기체를 따라 움직인다**(롤링 윈도우). 가장자리까지 여유가
`recenter_margin`(6 m) 아래로 떨어지면 창을 기체 중심으로 옮기고, 기존 데이터는
정수 셀 단위로 **시프트해서 보존**한다(새로 드러난 띠만 미탐색으로 비운다).
그래서 **월드 크기와 무관**해졌다.

```python
"rolling": True, "recenter_margin": 6.0,
"width": 300, "height": 300,          # 30 x 30 m 창
"origin_x": init_x - 15.0, ...        # 첫 창만 소환 좌표 중심
```

> 소수 셀만큼 옮기면 기존 데이터를 재보간해야 하고 그때마다 뭉개지므로 **정수 셀 단위로만**
> 움직인다. `np.roll` 은 돌아 나온 값을 반대편에 붙이므로 그 띠를 반드시 지워야 한다 —
> 안 지우면 **반대편 지형이 유령으로 남는다.**
>
> 결과: 탐색률 **0% → 26.9%** (합집합 기준. 층1 24.7%, 층2 17.7%). 창 원점이 기체를
> 따라 `(-61.2, 8.8)` 로 이동한 것도 확인했다.
>
> 🚨 **창은 지나온 곳을 잊는다 — 그래서 `/{ns}/map` 은 따로 누적한다.** Nav2 에는
> 창이 맞지만(현재 고도 주변만 보면 된다), 맵 병합과 탐사 판단에는 치명적이다.
> 드론이 창을 벗어나는 순간 그 영역이 `/map_merged` 에서 사라져 **커버리지가 되레
> 줄었다** (실측: 라운드마다 −6k, −18k, −15k 셀). 그러면 "전체를 다 탐사했다"를
> 잴 수가 없다.
>
> | 토픽 | 격자 | 쓰는 곳 |
> |---|---|---|
> | `/{ns}/map_active`, `/{ns}/map_layer_k` | 30×30 m @0.1 m **롤링 창** | Nav2, 고도 선택기 |
> | `/{ns}/map` | 100×100 m @0.2 m **누적** | 맵 병합, 탐사 판단 |
>
> 누적 격자에는 **미탐색(-1)을 반영하지 않는다.** 창이 지나간 자리는 미탐색으로
> 비워지는데 그걸 적으면 이미 본 것을 지우게 된다 — 고치려는 문제 그 자체다.
> 수정 후 `/map_merged` 가 단조 증가하는 것을 확인했다.
>
> **자율비행도 그 자리에서 통과했다** — `oneroom` 에서 드론 단독, `goal_pose_3d` 로
> 4.16 m 떨어진 목표를 주자 **4.35 m 비행해 0.20 m 이내 도달**(최근접 0.02 m),
> `/clock` 32.6 Hz(실시간 104%). 격자를 고치기 전에는 같은 목표가
> `Goal failed` 로 끝났다.
...
- **`3d` 모드는 없다.** 런치가 명시적으로 거부한다 ([0절](#0-경로계획-모드-고르기)).
...
- [08_DRONE_SETUP.md](08_DRONE_SETUP.md) — 기체 개조·제어 구조·센서 검증
- [Readme 11절](Readme.md#11-drone-중형급-쿼드콥터) — 빠른 사용법
- [01_INTERFACES.md](01_INTERFACES.md) — 토픽·프레임 규격
- [10_MAP_MERGE.md](10_MAP_MERGE.md) — 드론 맵이 병합에 들어가는 방법

---

← [08. 드론 구축](08_DRONE_SETUP.md) | [📖 책 목차](Readme.md#-목차) | [10. 맵 병합](10_MAP_MERGE.md) →
