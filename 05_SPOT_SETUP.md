...
### ⑧ 넓은 개활지에서 맵이 아예 안 생김 (`0x0`) ✅

**증상.** `oneroom`(96x96 m)에서 Spot 을 띄우면 `/spot1/map` 이 **0x0** 으로 남았다.
Nav2 는 `Received map message is malformed. Rejecting.` → `Robot is out of bounds of the
costmap!` 로 목표를 계속 실패시켰다. 좁은 월드에서는 멀쩡했다.

**추적.** 단계별로 재 보니 결함 위치가 한 곳으로 좁혀졌다.

| 단계 | 상태 |
|---|---|
| 뎁스 포인트클라우드 | ✅ 유효점 13,144 개 (0.7~5.6 m) |
| `base_link` ← 카메라 static tf | ✅ 10 개 전부 존재 |
| **`pointcloud_to_laserscan` 출력** | ❌ **723 점 전부 `inf`** |
| slam_toolbox | 노드 0 개 → 맵 `0x0` |

클라우드를 손으로 변환해 필터를 적용해 보니 **교집합이 공집합**이었다.

```
높이 통과 :  1,244 / 13,144   ← 전부 range_min 안쪽(자기 몸)
거리 통과 : 11,670 / 13,144   ← 전부 바닥 높이(base_link 기준 -0.49 m)
둘 다     :      0
```

**원인은 두 가지가 겹친 것이다.**

1. **개활지에는 `range_max`(10 m) 안에 벽이 없다.** 그래서 바닥 말고 반사가 없다.
   바닥은 `min_height` 로 (의도대로) 걸러지므로 남는 점이 0 이 된다.
2. **무반사를 `inf` 로 내보내고 있었다** (`use_inf: True`). `inf` 는 "측정 실패 = 모름"
   이라 slam_toolbox 가 **자유공간으로 지우지도 못한다.** 그래서 노드가 하나도 안 쌓였다.

**고친 방법 — 무반사를 "여기까지 비었다"로 바꾼다.**

```python
'use_inf': False,      # 무반사를 유한값으로
'inf_epsilon': 0.0,    # 정확히 range_max 로 (기본 1.0 이면 11 m 가 나가 가짜 장애물이 된다)
```

**단, 그것만으로는 위험하다.** `pointcloud_to_laserscan` 은 카메라 화각을 모르고
`-180~180` 전체를 채우므로, 그대로 두면 **카메라가 본 적 없는 방향까지 "비었다"고 선언**
한다. 그래서 [multi_scan_merger.py](src/webots_ros2_spot/webots_spot/multi_scan_merger.py)
에 **카메라별 각도 구역**을 넘겨, 구역 밖은 `inf`(미지)로 남긴다.

| 카메라 | 각도 구역 | 폭 |
|---|---|---|
| `right_flank_depth` | -144.4° ~ -47.6° | 96.7° |
| `left_head_depth` | -62.7° ~ 21.1° | 83.8° |
| `right_head_depth` | -21.6° ~ 63.4° | 85.0° |
| `left_flank_depth` | 47.6° ~ 144.0° | 96.4° |
| `rear_depth` | 139.0° ~ -138.9° (±180 을 걸침) | 82.1° |

값은 추측이 아니라 **`base_link` 로 변환한 실제 클라우드의 각도 분포를 재서** 얻었다.
다섯 구역의 합집합이 360도를 빈틈없이 덮는다.

병합기도 함께 고쳤다 — 예전에는 `laser_geometry.projectLaser` 로 클라우드를 만들어 담았는데
**그 함수가 `range_max` 이상인 값을 버려서** 무반사 정보가 거기서 또 사라졌다. 입력 스캔은
이미 `base_link` 기준이므로 각도를 그대로 빈에 매핑하도록 바꿨다.

**결과:** 병합 스캔 `0/723` → **`723/723` 유한값**, 맵 `0x0` → **201x201 (40,401 셀)**,
자율주행 목표 도달 확인 (2.05 m 이동, 오차 0.40 m).

> 실제 반사가 있는 빈은 무반사로 덮이지 않는다(장애물 우선). 겹치는 카메라가 있으면
> 병합에서 최소값을 취하므로 장애물이 이긴다.
